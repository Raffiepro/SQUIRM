...
void Serializer::SerializeToFile(std::string fileName, MachineCode assembly) {
  // Val i ara d'alguna manera doncs hem de parsear això. Anem a posar tot el
  // tema de constants
...
void Serializer::WriteInstruction(FILE *f, Instruction ins) {
  // Val de moment posarem les tres coses pero caldrà fer bit shifting segons el
  // tamany de les instruccions
  uint8_t i;
  int16_t a, b, c;
  i = (uint8_t)ins.type;
  a = (int16_t)ins.GetA();
...
bool Serializer::ReadInstruction(FILE *f,
  ...
  if (!fread(&i, sizeof(uint8_t), 1, f))
    return false;
  if (!fread(&a, sizeof(int16_t), 1, f))
    return false;
  if (!fread(&b, sizeof(int16_t), 1, f))
    return false;
  if (!fread(&c, sizeof(int16_t), 1, f))
    return false;
