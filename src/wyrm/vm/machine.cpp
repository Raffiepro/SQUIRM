...
void VM::Process(Instruction ins, int *current, std::vector<Atom *> *constants,
                 Atom **registers) {
  ...
  try {
    ...
    case InstructionID::LOADC: // A,B
      // if(debug) std::cout << "C: " << ins.GetB() << " (" <<
      // currentCode.constants[ins.GetB()].data.num << ") -> R: " << ins.GetA()
      // << std::endl;
      registers[ins.GetA()] =
          (*constants)[ins.GetB()]; // TODO: Aixo copia ? Crec que no
      break;
    ...
    case InstructionID::CALL: // A: func, B --- C: params
      // A veure aqui caldria eh aconseguir la funció i constants de A i doncs
      // executar recursivament i tal
      break;
    case InstructionID::BINARY:
      Operate((WyrmAPI::OpSymbol)ins.GetA(), registers[ins.GetB()],
              registers[ins.GetC()], registers[ins.GetD()]);
      break;
    default: // Nose excepcion supongo??? XD
      throw(
          std::string("Undefined instruction error " + std::to_string(tipus)));
      break;
    }
    if (!offseted && offset != 0)
      offset = 0;
  } catch (std::string r) {
    throw(r);
  }
...
void VM::Operate(WyrmAPI::OpSymbol op, Atom *a, Atom *b, Atom *c) {
  ...
  operations[Internal::HashOperation(WyrmAPI::OpType::OP_BINARY, op, a->typeId)]
      .binaryOperation(a->data, b->data, c->data);
  return;
...
std::string VM::GetOperationName(WyrmAPI::OpSymbol t) {
  std::map<WyrmAPI::OpSymbol, std::string> m = {
      {WyrmAPI::OP_ADD, "+"},  {WyrmAPI::OP_SUBTRACT, "-"},
      {WyrmAPI::OP_MUL, "*"},  {WyrmAPI::OP_DIV, "/"},
      {WyrmAPI::OP_MOD, "%"},  {WyrmAPI::OP_EQ, "=="},
      {WyrmAPI::OP_NEQ, "!="}, {WyrmAPI::OP_NOT, "!"},
      {WyrmAPI::OP_GT, ">"},   {WyrmAPI::OP_LT, "<"},
      {WyrmAPI::OP_GEQ, ">="}, {WyrmAPI::OP_LEQ, "<="},
      {WyrmAPI::OP_AND, "&&"}, {WyrmAPI::OP_OR, "||"}};
  return m[t];
}
